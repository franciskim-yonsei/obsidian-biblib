...
BibLib patches the `author` field inside Obsidian's Properties panel so structured author lists can be edited directly with an inline chip editor. You can click an author chip to edit it, use `×` to remove one, use `+` to add one, press `Enter` to confirm, press `Esc` to cancel, and click away to collapse the active inline form. Other nested CSL structures may still display warnings; that is a UI limitation, not a data problem. The YAML remains valid and can always be viewed in Source Mode.
